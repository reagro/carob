...
This report describes standardized data for:

`r m$data_citation`

These data were standardized on `r m$carob_date` by `r m$carob_contributor` with this `r script`. The original data are available `r http`. 
...
The observations are for `r nrow(locs)` locations for which we have `r nrow(uniqe(locs[, c("longitude", "latitude")])` unique coordinate pairs.
