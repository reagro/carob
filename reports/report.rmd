...
path <- ".."
f <- paste0(path, "/data/clean/", group, "/", suri, ".csv")
d <- read.csv(f) |> carobiner:::sort_by_terms("records", group, path)
...
get_values <- function(x, label) {
  x <- unique(x)
  ifelse(length(x)==1, x, paste0(length(x), " ", label, "(", paste(x, collapse=", "), ")"))
}
...
pub <- ifelse(is.na(m$publication), "", paste0('This is the accompanying <a href="', carobiner:::http_address(m$publication), "'>paper</a>"))
```


This report describes standardized data for `r m$title`. These data were standardized on `r m$carob_date` by `r m$carob_contributor` with this `r script`. The original data are available `r http`. This is the full citation: 
...
```{r tab2}
ktab(treatments, "treatments")
```

The fertilizer types used were: `r ferts`.

The observations are for `r nrow(locs)` locations for which we have `r nrow(unique(locs[, c("longitude", "latitude")]))` unique coordinate pairs.
