...
"Description:
	...
	dset <- data.frame(
	   ...
	   publication = NA,
	   data_citation = "Global Wheat Program; IWIN Collaborators; Singh, Ravi; Payne, Thomas, 2019, '11th High Rainfall Wheat Yield Trial', https://hdl.handle.net/11529/10548202, CIMMYT Research Data & Software Repository Network, V2, UNF:6:8B2lZ7f/GAnAmOz9N/I9GA== [fileUNF]",
...
## download and read data 
	...
	env <- ff[basename(ff) == "12TH HRWYT_EnvData.xls"]
	geno <- ff[basename(ff) == "12TH HRWYT_Genotypes_Data.xls"]
	grn <- ff[basename(ff) == "12TH HRWYT_GrnYld.xls"]
	loc <- ff[basename(ff) == "12TH HRWYT_Loc_data.xls"]
	raw <- ff[basename(ff) == "12TH HRWYT_RawData.xls"]
...
# other variables
	...
	renv$USE_OF_FIELD_SPECIFY_CROP <- ifelse(renv$USE_OF_FIELD_SPECIFY_CROP == "OIL CROP", "rapeseed", renv$USE_OF_FIELD_SPECIFY_CROP)
	...
	renv$USE_OF_FIELD_SPECIFY_CROP <- ifelse(renv$USE_OF_FIELD_SPECIFY_CROP == "CORN", "maize", renv$USE_OF_FIELD_SPECIFY_CROP)
		...
		renv$USE_OF_FIELD_SPECIFY_CROP <- ifelse(renv$USE_OF_FIELD_SPECIFY_CROP == "LEGUMES", "common bean", renv$USE_OF_FIELD_SPECIFY_CROP) # Update once I get clarification
		renv$USE_OF_FIELD_SPECIFY_CROP <- ifelse(renv$USE_OF_FIELD_SPECIFY_CROP == "LEGUME", "common bean", renv$USE_OF_FIELD_SPECIFY_CROP) # Update once I get clarification
		renv$USE_OF_FIELD_SPECIFY_CROP <- ifelse(renv$USE_OF_FIELD_SPECIFY_CROP == "V. RADIATA MOONG", "mung bean", renv$USE_OF_FIELD_SPECIFY_CROP) # Update once I get clarification
		renv$USE_OF_FIELD_SPECIFY_CROP <- ifelse(renv$USE_OF_FIELD_SPECIFY_CROP == "PAPPER CROP", "pepper", renv$USE_OF_FIELD_SPECIFY_CROP) # Update once I get clarification
...
renv$USE_OF_FIELD_SPECIFY_CROP <- ifelse(renv$USE_OF_FIELD_SPECIFY_CROP == "TRIGO", "fenugreek", renv$USE_OF_FIELD_SPECIFY_CROP)
	...
	renv$USE_OF_FIELD_SPECIFY_CROP <- ifelse(renv$USE_OF_FIELD_SPECIFY_CROP == "MILICIMA ATERRINA", "mucuna", renv$USE_OF_FIELD_SPECIFY_CROP)
				renv$USE_OF_FIELD_SPECIFY_CROP <- ifelse(renv$USE_OF_FIELD_SPECIFY_CROP == "RAPHANUS  SPP", "vegetables", renv$USE_OF_FIELD_SPECIFY_CROP) # Update once I get clarification
