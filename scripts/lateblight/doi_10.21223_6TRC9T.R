...
carob_script <- function(path) {
   ...
   r <- carobiner::read.excel(ff[basename(ff)=="PTLate blight092014_OXAPMP_exp8.xlsx"],sheet = "Fieldbook")
   
   d <- data.frame(
      crop="potato",
      variety= r$INSTN,
      rep= as.integer(r$REP),
      yield=(r$TTWP/2.7 )*10000, # to kg/ha
      AUDPC= r$AUDPC/100,
      on_farm= TRUE,
      irrigated= NA,
      inoculated= FALSE,
   ...
   r1 <- data.frame(rbind(m$Value))
   names(r1) <- m$Factor
   d$country <- r1$Country
   d$adm1 <- r1$Admin1
   d$adm2 <- r1$Admin2
   d$adm2 <- r1$Admin2
   d$adm3 <- r1$Admin3
   d$location <- r1$Locality
   d$latitude <- as.numeric(r1$Latitude)
   d$longitude <- as.numeric(r1$Longitude)
   d$elevation <- as.numeric(r1$Elevation)
   d$planting_date <- as.character(as.Date(r1$Begin_date, "%d/%m/%Y"))
   d$harvest_date <- as.character(as.Date(r1$End_date, "%d/%m/%Y"))
   d$plant_spacing <- 30 
   d$row_spacing <- 90
