...
"
  ...
  dataset_id <- carobiner::simple_uri(uri)
	...
	dset <- data.frame(
	   ...
	   publication = NA,
	   data_citation = "Global Wheat Program; IWIN Collaborators; Singh, Ravi; Payne, Thomas, 2017, '35th Elite Selection Wheat Yield Trial', https://hdl.handle.net/11529/10988, CIMMYT Research Data & Software Repository Network, V5, UNF:6:2IKKDwQUC+x31JkKRTnqjA== [fileUNF]",
...
## download and read data 
	...
	js <- carobiner::get_metadata(dataset_id, path, group, major=5, minor=1)
...
## process file(s)
	...
	i <- which(d$location == "Choryn, Danco LTD")
	d$latitude[i] <- 52.05
	
	#much too low
	d$heading[d$country == "South Africa"] <- NA
