...
# InorgP_Cnt, InorgP_Trt, InorgP_Trt.1, MGF_1_Description_Trt,
# Old_SoilType
# Organic_matter_added, Organic_matter_type,
# Parent.material, pH_CaCl2, PH_Method, pHClass, Phenols.ug.g._Trt, Phyticphosphorus.mg.g._Trt, 
# SOC.Clay.rati, SOCclass, Soil.type_New, Treat_code, Treatment.corrected,
# X.If.yes..form.of.Zn.application..Soil..Foliar..,
# Zn..kg.ton., Zn_applied..Yes..No.., Zn_applied_.Kg.ha., Zn_method, ZntoPratio
	...
	d$site <- iconv(d$site, "UTF-8", "UTF-8")
	d$site <- trimws(gsub("\"", "", d$site))
	...
	d$Zn_fertilizer[d$Zn_fertilizer == "Yes"] <- NA
	d$Zn_fertilizer <- as.numeric(d$Zn_fertilizer)

	d$OM_used = !is.na(r$FYM_Trt)
	d$OM_amount <- r$FYM_Trt
	d$OM_amount[d$OM_amount == "Yes"] <- NA
	d$OM_amount <- as.numeric(d$OM_amount)
	d$OM_type[d$OM_used] <- "farmyard manure"
