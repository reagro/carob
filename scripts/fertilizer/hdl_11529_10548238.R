...
carob_script <- function(path) {
   ...
   d2$harvest_date= as.character(as.Date(d2$planting_date) +120) ## harvest date was around 120 days after planting
   ## subset Soil data 
   d3 <- data.frame(
   ...
   #Fixing harvest date
   d$harvest_date <- gsub("2017-09-30","2016-09-30",d$harvest_date) ## Maize can not be harvest after more than 1 an after planting. it's probably an entry error 
