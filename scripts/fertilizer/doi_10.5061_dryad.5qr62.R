...
carob_script <- function(path) {
   ...
   Initial soil N pools were larger in Yala than Tumbi (240 vs. 79 kg/ha). Inorganic N pools did not change in Yala (277 kg/ha), but increased fourfold after cultivation and fertilization in Tumbi (371 kg/ha). Intra-annual variability in NO−3-N concentrations (3–33 μg/g) in Tumbi topsoils strongly 
   
   suggested that the sandier soils were prone to high leaching losses.Information on soil inorganic N pools and movement through soil profiles can h vulnerability of SSA croplands to N losses and determine best fertilizer management practices as N application rates increase. A better understanding 
   
   of the vertical and temporal patterns of soil N pools improves our ability to predict the potential environmental effects of a dramatic increase in fertilizer application rates that will accompany the intensification of African croplands. " 
   ...
   dset <- data.frame(
      ...
      data_institute = "UM", #Univeristy of Maryland" 
   ...
   process <- function(f){
      ...
      if (is.null(r$pH)) r$pH <- as.numeric(NA) ; if (is.null(r$Depth)) r$Depth <- as.numeric(NA)
      if (is.null(r$Location)) r$Location <- as.numeric(NA) ; if (is.null(r$BD)) r$BD <- as.numeric(NA)
      if (is.null(r$DepthMid)) r$DepthMid <- as.numeric(NA) ; if (is.null(r$kgNperha)) r$kgNperha <- as.numeric(NA)
      d <- data.frame(
         country= r$Location,
         soil_pH= r$pH,
   ...
   d$longitude[d$location=="Yala"] <- 34.5338279 ; d$latitude[d$location=="Yala"] <- 0.0954435
   d$longitude[d$location=="Tumbi"] <- 34.6292496 ; d$latitude[d$location=="Tumbi"] <- -11.0334746
   
   ###
   d$Deph <- gsub("to","-",d$Deph)
   k <- t((data.frame(strsplit(d$Deph,'-')))) ## transpose 
   ...
   d$date[j] <- format(as.Date(d$date[j],"%d/%m/%y"), "%Y-%m-%d")
   i <- grep("WeeklyKCl",d$trial_id)
   d$date[i] <- format(as.Date(d$date[i], "%d-%m-%y"),"%Y-%m-%d")
   d$year  <- substr(d$date,1,4)
   
   ## adding yield data ( annual yield ) ## from doi:10.1890/15-1518.1
   yd <- data.frame( treatment= c("control", "control", "200N", "control", "200N", "control", "control", "75", "200N"),
   ...
   d$Deph <- gsub(" - | -","-",d$Deph)
   soil <- data.frame(Deph=c(rep(c("0-15", "15-30", "30-50", "50-100", "100-150", "150-200", "200- 250", "250-300", "300-350", "350-400"),2)),
   ...
   d <- merge(d,soil,by=c("Deph","country"),all.x = TRUE)
   
   d$k <- ifelse(d$country=="Kenya" & d$Deph=="0-15",1,
                 ifelse(d$country=="Tanzania" & d$Deph=="0-15", 2,NA))
