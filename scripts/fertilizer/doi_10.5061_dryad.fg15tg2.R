...
"
	...
	dd <- data.frame(
		SY1= c("Eldoret14","Embu ATC15","EmbuKPS15","Kandara14","Kandara15SR","Kisii14","Kitale15","Machakos","Njoro14"),
	...
	dd1 <- data.frame(
		dd = c("2014B Ngoma","2014B Nyagatare","2015 B Ngoma","2015 B Nyagatare","2015 B Musanze","2015A Bugesera", "2015A Ngoma","2015A Nyagatare","2015A Burera","2015A Musanze","2015A Huye"),
	...
	d2 <- d2[, c("trial_id","country","adm2","latitude","longitude","crop","planting_date","harvest_date","variety", "rep", "treatment","N_fertilizer","P_fertilizer","K_fertilizer","elevation", "soil_P_total", "soil_K","yield")]
	
	d3$country <- "Tanzania"
	v <- d3$S
	v <- carobiner::replace_values(v, c("Mav","Daredo"), c("Mavamizi","Dareda"))
	d3$adm3 <- v
	d3$rep <- d3$R
	v <- d3$Variety
	v <- carobiner::replace_values(v, 
...
##	d3[is.na(d3)] <- 0
	...
	d3 <- d3[,c("trial_id","country","adm3","latitude","longitude","crop","variety","rep","planting_date","harvest_date","N_fertilizer","P_fertilizer","K_fertilizer", "elevation","soil_pH","soil_P_total","soil_K","yield")]
	...
	d4$crop <- "maize"
	v <- d4$Variety
	v <- carobiner::replace_values(v, c("ZM606","ZM521"), c("ZM 606","ZM 521"))
	d4$variety <- v
	d4$rep <- d4$R
	...
	LL <- data.frame(
		site = c("Mt. Makulu","Msekera","Choma","Kasama"),
	...
	d4 <- d4[, c("trial_id","country","site","latitude","longitude","crop","variety","rep","planting_date","harvest_date","elevation", "soil_pH","soil_P_total","soil_K","N_fertilizer","P_fertilizer","K_fertilizer","yield")]
	...
	z$harvest_date <- ifelse(grepl("-", z$harvest_date),
		 as.character(as.Date(z$harvest_date, format = c("%Y-%m-%d"))), NA)
