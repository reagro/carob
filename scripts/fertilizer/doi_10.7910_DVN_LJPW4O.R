...
## download and read data 
	...
	d$start_date <- as.numeric("2013")
	d$end_date <- as.numeric("2014")
	d$on_farm <- "no"
	d$is_survey <- "no"
	d$crop <- "maize"
	# d$treatment <- "Soil nutrient available"
	...
	biomass$season <- ifelse(biomass$`Days after planting (dap)` == 30, "dry", "rainy")
  biomass <- biomass[order(biomass[,"Site"], biomass[,"Season"], biomass[,"Block"], biomass[,"Treatment"]), ]
	...
	# Biomass is not making too much sense compared to yield
	d1$biomass_total <- ((d1$`Dry weight with roots (g)` * 8) * (100/5.625))/1000 # 5a Plant growth data.xlsx --> (2 rows/plot * 20 plants/row) / 5 plants sampled
	...
	d <- carobiner::change_names(d2,
	                             c("country", "adm1", "adm2", "adm3", "latitude", "longitude", "Site", "start_date", "end_date", "season", "on_farm", "is_survey", "crop", "biomass_total", "yield", "residue_yield", "pH", "N (%)", "K (ppm)", "P (ppm)", "Sand (%)", "Clay (%)"),
	                             c("country", "adm1", "adm2", "adm3", "latitude", "longitude", "site", "start_date", "end_date", "season", "on_farm", "is_survey", "crop", "biomass_total", "yield", "residue_yield", "soil_pH", "soil_N", "soil_K", "soil_P_total", "soil_sand", "soil_clay"))
	d <- d[,c("country", "adm1", "adm2", "adm3", "latitude", "longitude", "site", "start_date", "end_date", "season", "on_farm", "is_survey", "crop", "biomass_total", "yield", "residue_yield", "soil_pH", "soil_N", "soil_K", "soil_P_total", "soil_sand", "soil_clay")]
