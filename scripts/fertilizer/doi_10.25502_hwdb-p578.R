...
old_carob_script <- function(path){
...
#	lat_lon1 <- d[is.na(d$gps_latitude) | is.na(d$gps_longitude), ]
 ...
 # creating a data frame with the unique entries and the latitudes and longitudes, coordinates hard coded
    ...
    d <- d[, c("trial_id","country","adm2", "location","latitude","longitude")]
	...
	v <- carobiner::replace_values(dd$crop, 
		c("Groundnuit","Groudnuit","","Soybean","Soybean (Farmer variety)","Soyben","Soybean PD1","Soybean PD2"),	 
	...
	q4 <- q4[, c("trial_id","country","adm2","location", "latitude","longitude","rep","crop","variety","inoculated","fertilizer_type","N_fertilizer","P_fertilizer","K_fertilizer","planting_date","harvest_date", "row_spacing","plant_spacing","yield","previous_crop","dataset_id")]
