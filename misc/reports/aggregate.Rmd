...
noxy <- is.na(xy$latitude) | is.na(xy$longitude)
...
if (nrow(xy) > 0) {
	...
	haveXY <- TRUE
	xy$dataset_id <- paste0("<a href='https://carob-data.org/reports/", xy$dataset_id, ".html'>", xy$dataset_id, "</a>")
} else {
...
This dataset combines `r nsources` standardized individual data [sources](https://carob-data.org/done.html). It has has `r nrow(d)` records and `r ncol(d)` variables for `r length(crop)` crops and `r length(country)` countries. `r vartxt`
