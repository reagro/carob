...
d <- read.csv(f) 
m <- read.csv(gsub(".csv$", "_metadata.csv", f))
...
This dataset combines `n nsources` standardized individual data [sources](https://carob-data.org/done.html). It has has `r nrow(d)` records and `r ncol(d)` variables for `r crop` in `r country`. `r vartxt`
